...
import type { ApiGroup, BatchOprCommand, BatchSaveCommand, CountApi, ListApi, PageResult, RequestResult, QueryTemplate } from '#v/types'
import { uuid, cloneJson } from '#v/utils'
import { useFetch, useNuxtApp, type UseFetchOptions } from 'nuxt/app'

type FetchResult<T> = Promise<{ data: Ref<RequestResult<T>> }>
...
// 根据useApiFetch封装
export async function usePostFetch<T>(url: string, data?: any, customOptions: UseFetchOptions<T> = {}): FetchResult<T> {
  // 不clone的话，会导致data被修改，会出发请求，不知道为啥
  return await useApiFetch<T>(url, { method: 'POST', body: cloneJson(data), ...customOptions })
}
export async function usePutFetch<T>(url: string, data?: any, customOptions: UseFetchOptions<T> = {}): FetchResult<T> {
  // 不clone的话，会导致data被修改，会出发请求，不知道为啥
  return await useApiFetch<T>(url, { method: 'PUT', body: cloneJson(data), ...customOptions })
}
export async function useGetFetch<T>(url: string, customOptions: UseFetchOptions<T> = {}): FetchResult<T> {
  return await useApiFetch<T>(url, { method: 'GET', ...customOptions })
}
export async function useDeleteFetch<T>(url: string, customOptions: UseFetchOptions<T> = {}): FetchResult<T> {
  return await useApiFetch<T>(url, { method: 'DELETE', ...customOptions })
}

// 根据上述方法封装API调用函数
export function useApiCreate<M>(basePath: string): (model: M, customOptions?: UseFetchOptions<M>) => FetchResult<M> {
  return function (model: M, customOptions: UseFetchOptions<M> = {}) {
    return usePostFetch<M>(basePath, model, customOptions)
  }
}
export function useApiBatchCreate<M>(basePath: string): (cmd: BatchSaveCommand<M>, customOptions?: UseFetchOptions<M[]>) => FetchResult<M[]> {
...
export function useApiUpdate<M>(basePath: string): (model: M, customOptions?: UseFetchOptions<M>) => FetchResult<M> {
  return function (model: M, customOptions: UseFetchOptions<M> = {}) {
    return usePutFetch<M>(basePath, model, customOptions)
  }
}
export function useApiBatchUpdate<M>(basePath: string): (cmd: BatchSaveCommand<M>, customOptions?: UseFetchOptions<M[]>) => FetchResult<M[]> {
  return function (cmd: BatchSaveCommand<M>, customOptions: UseFetchOptions<M[]> = {}) {
    return usePutFetch<M[]>(`${basePath}/batch`, cmd, customOptions)
  }
}
export function useApiGetById<M>(basePath: string): (id: number, customOptions?: UseFetchOptions<M>) => FetchResult<M> {
  return function (id: number, customOptions: UseFetchOptions<M> = {}) {
    return useGetFetch<M>(`${basePath}/${id}`, customOptions)
  }
}
export function useApiGetByString<M>(basePath: string): (param: string, customOptions?: UseFetchOptions<M>) => FetchResult<M> {
  return function (param: string, customOptions: UseFetchOptions<M> = {}) {
    return useGetFetch<M>(`${basePath}/${param}`, customOptions)
  }
}
export function useApiDeleteById(basePath: string): (id: number, customOptions?: UseFetchOptions<void>) => FetchResult<void> {
  return function (id: number, customOptions: UseFetchOptions<void> = {}) {
    return useDeleteFetch(`${basePath}/${id}`, customOptions)
  }
}
export function useApiBatchDelete(basePath: string): (delReq: BatchOprCommand, customOptions?: UseFetchOptions<void>) => FetchResult<void> {
...
export function useApiCount<M>(basePath: string): CountApi<M> {
...
export function useApiList<M>(basePath: string): ListApi<M> {
...
export function useApiCountAndList<M>(basePath: string): ListApi<M> {
  return function (payload: Omit<QueryTemplate<M>, 'selectQuery'>, customOptions: UseFetchOptions<PageResult<M>> = {}) {
